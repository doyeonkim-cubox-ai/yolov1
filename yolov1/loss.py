...
class YOLOLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        iou, best_box = torch.max(ious, dim=0)
        Iobj_i = target[..., 4:5]
        ...
        bbox_loss = self.loss_fn(torch.flatten(bbox_pred, end_dim=-2),
                                 torch.flatten(bbox_target, end_dim=-2))

        # confidence loss
        obj_pred = ((1 - best_box) * pred[..., 4:5] + best_box * pred[..., 9:10])
        obj_target = target[..., 4:5]
        obj_loss = self.loss_fn(torch.flatten(Iobj_i * obj_pred),
                                torch.flatten(Iobj_i * obj_target))
        noobj_loss = self.loss_fn(torch.flatten((1 - Iobj_i) * pred[..., 4:5], start_dim=1),
                                  torch.flatten((1 - Iobj_i) * obj_target, start_dim=1))
        noobj_loss += self.loss_fn(torch.flatten((1 - Iobj_i) * pred[..., 9:10], start_dim=1),
                                   torch.flatten((1 - Iobj_i) * obj_target, start_dim=1))

        # class probability loss
        class_pred = pred[..., 10:]
        class_target = target[..., 10:]
        class_loss = self.loss_fn(torch.flatten(Iobj_i * class_pred, end_dim=-2),
